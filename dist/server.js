...
async function connectToSlack() {
    ...
    Slack.onmessage = msg => {
        ...
        if (parsedMessage.type === 'interactive') {
            ...
            sendMessageToSlack(responseUrl, message.ok
            ...
            message.ok &&
                cypress(message.url, message.type)
                    ...
                    sendMessageToSlack(responseUrl, `:robot_face: Your test result is now available @  **http://192.168.50.233:3000/ftp/CypressTests/${parsedMessage.envelope_id}**. \n It contains a written summary of the test as well as screenshots and videos of failed tests :)`);
                })
